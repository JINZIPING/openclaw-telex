...
export const TelexToolSchema = Type.Union([
	...
	Type.Object({
		action: Type.Literal("update_conversation_settings", {
			description:
				"Change a channel's settings: allow an action to every member, deny it to restrict it to the owner and admins, and replace the announcement. Only the owner and admins may change permissions; the announcement follows the channel's announcement permission.",
		}),
		conversation_id: Type.String({ description: "Conversation id (16-char hex)" }),
		allow: Type.Optional(
	...
	Type.Object({
		action: Type.Literal("delete_conversation", {
			description: "Delete a channel the bot owns.",
		}),
		conversation_id: Type.String({ description: "Conversation id (16-char hex)" }),
	}),
	Type.Object({
	...
	Type.Object({
		action: Type.Literal("remove_members", {
			description:
				"Remove members from a channel by identity id and/or email; every email must resolve or nobody is removed. The owner cannot be removed.",
		}),
		conversation_id: Type.String({ description: "Conversation id (16-char hex)" }),
		identity_ids: Type.Optional(
			Type.Array(Type.String(), { description: "Member identity ids (16-char hex)" }),
	...
	Type.Object({
		action: Type.Literal("update_member_role", {
			description:
				"Set a channel member's role. Owner and admins only; 'owner' is owner only, hands the channel over and leaves you an admin.",
		}),
		conversation_id: Type.String({ description: "Conversation id (16-char hex)" }),
